...
import { useProjectBySlug } from '../lib/payload';
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, Cell } from 'recharts';

// Basic serializer for Payload's Lexical JSON
const serializeLexical = (node: any): React.ReactNode => {
    ...
    if (node.type === 'paragraph') {
        return (
            <p className="mb-4 text-lg text-neutral-600 leading-relaxed font-light">
    ...
    if (node.type === 'list') {
        ...
        return (
            <Tag className={`${listClass} pl-5 mb-4 text-lg text-neutral-600 leading-relaxed font-light`}>
                {node.children?.map((child: any, i: number) => (
                    <React.Fragment key={i}>{serializeLexical(child)}</React.Fragment>
                ))}
            </Tag>
    ...
    if (node.type === 'listitem') {
        // Check if the list item has visible text.
        // If it only contains a nested list (and whitespace), we should hide the parent bullet to avoid "double dots".
        const hasText = node.children?.some((child: any) => child.type === 'text' && child.text && child.text.trim().length > 0);
        const hasList = node.children?.some((child: any) => child.type === 'list');

        // If it has a list but no text, remove the list marker (list-none).
        const className = (hasList && !hasText) ? "mb-1 list-none" : "mb-1";
    ...
    if (node.root && node.root.children) {
        return (
            <>
                {node.root.children.map((child: any, i: number) => (
                    <React.Fragment key={i}>{serializeLexical(child)}</React.Fragment>
                ))}
            </>
        );
...
export const ProjectDetail: React.FC = () => {
    ...
    const seoTitle = project ? project.title : "Project Detail";
    const seoDesc = project ? project.summary : "Detailed case study.";
    const seoImage = project ? (typeof project.coverImage === 'string' ? project.coverImage : project.coverImage?.url) : undefined;
        ...
        <Layout>
            {project && (
                <SEO
                    ...
                    image={typeof project.coverImage === 'string' ? project.coverImage : undefined}
                    url={`https://ezekiel.ai/projects/${project.slug}`}
                />
            )}
            <article className="pt-32 pb-24">
                ...
                {/* Content Body */}
                <div className="max-w-6xl mx-auto px-4 md:px-6 grid md:grid-cols-12 gap-12">
                    ...
                    <div className="md:col-span-8 space-y-16">
                        <FadeIn>
                            ...
                            <div>
                                {serializeLexical(project.problem)}
                            </div>
                        </FadeIn>
                        ...
                        <FadeIn delay={0.1}>
                            ...
                            <div className="mb-6">
                                {serializeLexical(project.solution)}
                            </div>
                        ...
                        {project.chartData && project.chartData.length > 0 && (
                            <FadeIn delay={0.2}>
                                ...
                                <div className="w-full bg-neutral-50 rounded-xl p-6 border border-neutral-100">
                                            ...
                                            <BarChart data={project.chartData} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                                                ...
                                                <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                                                    {project.chartData.map((entry, index) => (
                                                        <Cell key={`cell-${index}`} fill={entry.type === 'after' ? '#2563eb' : '#9333ea'} />
                                                    ))}
                                                </Bar>
                                            </BarChart>
                                        </ResponsiveContainer>
                                    </div>
                                    <p className="text-center text-xs text-neutral-400 mt-4">Comparison: Before vs After</p>
                                </div>
                            </FadeIn>
                        )}
                    </div>
                    ...
                    <div className="md:col-span-4 space-y-12">
                        ...
                        <FadeIn delay={0.3}>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {project.tags?.map((tagItem: any) => (
                                    <span key={tagItem.tag || tagItem} className="px-3 py-1 bg-neutral-100 text-neutral-600 rounded text-sm">
                                        {tagItem.tag || tagItem}
                                    </span>
                                ))}
                            </div>
                ...
                {/* CTA */}
                <div className="max-w-6xl mx-auto px-4 md:px-6 mt-24 pt-12 border-t border-neutral-100 text-center">
